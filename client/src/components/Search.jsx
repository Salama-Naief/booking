...
export default function Search({setUserInfo,userInfo,setState,state}) {
  ...
  return (
    <div className='flex flex-col bg-yellow-500 p-4 rounded sticky top-0 left-0 h-max '>
            <div className='text-xl text-gray-700 py-2 font-bold'>{t("search")}</div>
            <div className=''>
                <div className="text-sm text-gray-700">{t("destination")}</div>
            <input type="text" className=" bg-gray-100 py-2 px-4 outline-none w-full cursor-pointer rounded" placeholder=" Your distenation"/>
            <div className="text-sm mt-4 text-gray-700">{t("checkeddate")}</div>
         <div className="relative  bg-gray-100 py-2 px-4 rounded ">
         <div onClick={()=>setDropDate(!dropDate)} className="cursor-pointer">{`${format(state[0].startDate,"dd/MM/yyyy")}`} {t("to")} {`${format(state[0].endDate,"dd/MM/yyyy")}`}</div>
         ...
         <div className="text-sm mt-4 text-gray-700">{t("options")}</div>
         <div className="relative bg-gray-100 py-2 mb-4 px-4 cursor-pointer rounded"> 
         <div className=" cursor-pointer" onClick={()=>setDropInfo(!dropInfo)}>{`  ${userInfo.adults} ${t("adults")}, ${userInfo.children} ${t("children")}, ${userInfo.rooms} ${t("rooms")}`} </div>
           <div className={`${dropInfo?"block":"hidden"} absolute w-full px-1 bg-white left-0 top-11`} >
                 <div className="flex justify-between m-4">
                    <div className="w-2/3">{t("adults")}</div>
                 ...
                 <div className="flex justify-between m-4">
                    <div className="w-2/3">{t("children")}</div>
                    <div className="w-1/3 flex justify-between items-center">
          ...
          <div>
              <div className='flex items-center mb-2'>
                  <input type="checkbox" />
                  <div className='text-sm text-gray-700 pl-1'>{t("entirehome")}</div>
              </div>
              <div className='flex items-center mb-2'>
                  <input type="checkbox" />
                  <div className='text-sm text-gray-700 pl-1'>{t("workingtravling")}</div>
              </div>
          </div>
          <button className="px-3 w-full text-white py-2 mt-2 bg-blue-600 border-2 border-yellow-500 rounded">{t("search")}</button>
            </div>
        </div>
  )
}
